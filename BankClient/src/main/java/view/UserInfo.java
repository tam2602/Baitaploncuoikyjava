...
//	}
	...
	public UserInfo() {
		...
		btnRuttien.addActionListener(e->{
			if(getUserPass().equals(getUserRepeatPass())) {
				Socket socket= ViewLogin.getSocket();
				InforListener listenner= new InforListener(socket, this);
				try {
					boolean check=listenner.EditInfo();
					if(check) {
						UserAccount useredit= UserView.getUsermain();
						useredit.setFullName(getUserName());
						useredit.setPassword(getUserRepeatPass());
						UserView.setUser(useredit);
		...
		lblHVT.setFont(new Font("Times New Roman", Font.BOLD, 25));
		lblHVT.setBounds(10, 296, 194, 70);
		contentPane.add(lblHVT);
		
		textFieldName = new JTextField();
		textFieldName.setFont(new Font("Times New Roman", Font.PLAIN, 20));
		textFieldName.setBounds(327, 299, 427, 73);
		...
		textFieldPass.setFont(new Font("Times New Roman", Font.PLAIN, 20));
		textFieldPass.setColumns(10);
		textFieldPass.setBounds(327, 419, 427, 73);
		...
		textFieldRePass.setFont(new Font("Times New Roman", Font.PLAIN, 20));
		textFieldRePass.setColumns(10);
		textFieldRePass.setBounds(327, 520, 427, 73);
	...
	public String getUserPass() {
		return textFieldPass.getText().trim();
	}
	public String getUserRepeatPass() {
		return textFieldRePass.getText().trim();
	}
}
